...
import logging
from const import  DOCUMENT_TYPE
import time
...
def run_scrape(start, end, document_types):
    ...
    for type in document_types:
        site_lists += DOCUMENT_TYPE[type]
    for site in site_lists:
        print(site)
        response = requests.get(
            set_url_params(1, convert_date(start), convert_date(end), site),
            headers=headers,
        )
        if response.status_code == 200:
            logging.info(
                f"Page Load SUCCESS URL: {set_url_params(1, convert_date(start), convert_date(end), site)}"
            )
            ...
            for page in range(1, page_count + 1):
                response = requests.get(
                    set_url_params(page, convert_date(start), convert_date(end), site),
                    headers=headers,
                )
                if response.status_code == 200:
                    logging.info(
                        f"Page Load SUCCESS URL: {set_url_params(page, convert_date(start), convert_date(end), site)}"
                    )
                    soup = BeautifulSoup(response.content, "html.parser")
                    ...
                    logging.error(
                        f"Page Load FAILED URL: {set_url_params(page, convert_date(start), convert_date(end), site)}"
                    )
                time.sleep(3)
        else:
            logging.error(
                f"Page Load FAILED URL: {set_url_params(1, convert_date(start), convert_date(end), site)}"
            )
    generate_json()
